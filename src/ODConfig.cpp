...
ODConfig::ODConfig(const wxString& appName, const wxString& vendorName,
  ...
  // ODConfig *pCF = new MyConfig::MyConfig( wxString( _T("") ), wxString(
  // _T("") ), gConfig_File ); g_pODConfig->LoadMyConfig( 0 );
...
bool ODConfig::UpdatePath(ODPath* pb) {
  ...
  if (!m_bSkipChangeSetUpdate) {
    if (pb->m_bSaveUpdates) m_pODNavObjectChangesSet->AddPath(pb, "update");
  }
...
bool ODConfig::AddNewODPoint(ODPoint* pODP, int crm) {
  if (pODP->m_bIsInLayer) return true;

  if (!pODP->m_bIsolatedMark) return true;
...
bool ODConfig::ExportGPXPaths(wxWindow* parent, PathList* pPaths,
                              const wxString suggestedName) {
  wxFileDialog saveDialog(NULL, _("Export GPX file"), m_sGPX_Path,
                          suggestedName, wxT("GPX files (*.gpx)|*.gpx"),
                          wxFD_SAVE);
  ...
  if (response == wxID_OK) {
    ...
    if (wxFileExists(fn.GetFullPath())) {
      int answer = OCPNMessageBox_PlugIn(
          parent, _("Overwrite existing file?"), _("Confirm"),
...
bool ODConfig::ExportGPXODPoints(wxWindow* parent, ODPointList* pODPoints,
                                 const wxString suggestedName) {
  wxFileDialog saveDialog(NULL, _("Export GPX file"), m_sGPX_Path,
                          suggestedName, wxT("GPX files (*.gpx)|*.gpx"),
                          wxFD_SAVE);
      ...
      if (file_array.GetCount()) {
        ...
        if (file_array.GetCount() <= 1)
          wxFileName::SplitPath(file_array[0], NULL, NULL,
                                &(l_lLayer->m_LayerName), NULL, NULL);
        else
          wxFileName::SplitPath(filename, NULL, NULL, &(l_lLayer->m_LayerName),
                                NULL, NULL);
        ...
        wxString laymsg;
        laymsg.Printf(wxT("New layer %d: %s"), l_lLayer->m_LayerID,
                      l_lLayer->m_LayerName.c_str());
        wxLogMessage(laymsg);
        ...
        for (unsigned int i = 0; i < file_array.GetCount(); i++) {
          ...
          if (::wxFileExists(file_path)) {
            ...
            long nItems =
                pSet->LoadAllGPXObjectsAsLayer(l_lLayer->m_LayerID, bLayerViz);
...
// #ifdef COMPILER_SUPPORTS_CXX11
    ...
    while (node) {
      ...
      if (bviz_only && !podp->m_bIsVisible) b_add = false;

      if (podp->m_bIsInLayer && !blayer) b_add = false;
      if (b_add) {
    ...
    while (node1) {
      ...
      if (bviz_only && !pPath->IsVisible()) b_add = false;

      if (pPath->m_bIsInLayer && !blayer) b_add = false;
...
void ODConfig::UI_ImportGPX(wxWindow* parent, bool islayer, wxString dirpath,
                            bool isdirectory) {
  ...
  if (!islayer || dirpath.IsSameAs(_T(""))) {
    ...
    response = PlatformFileSelectorDialog(
        NULL, &selectedFile, _("Import GPX file"), m_sImport_Path, wxT(""),
    ...
    wxFileDialog openDialog(NULL, _("Import GPX file"), m_sImport_Path, wxT(""),
                            wxT("GPX files (*.gpx)|*.gpx|CSV files "
                                "(*.csv)|*.csv|All files (*.*)|*.*"),
    ...
    if (islayer) {
      ...
      if (file_array.GetCount() <= 1)
        ...
        if (dirpath.IsSameAs(_T("")))
          wxFileName::SplitPath(m_sImport_Path, NULL, NULL, &(l->m_LayerName),
                                NULL, NULL);
        else
      ...
      wxString laymsg;
      if (l_foundLayer) {
        laymsg.Printf(wxT("Layer %d: %s is a duplicate"), l->m_LayerID,
                      l->m_LayerName.c_str());
      } else {
    ...
    for (unsigned int i = 0; i < file_array.GetCount(); i++) {
      ...
      if (wxFileExists(path)) {
        ...
        if (m_sImport_Type == _T("gpx")) {
          ...
          if (islayer) {
            l->m_NoOfItems +=
                pSet->LoadAllGPXObjectsAsLayer(l->m_LayerID, l->m_bIsVisible);
          } else
            pSet->LoadAllGPXObjects(
                true);  // Import with full vizibility of names and objects
        } else {
          if (islayer) {
            l->m_NoOfItems += pSet->Load_CSV_File(path.fn_str(), l->m_LayerID,
                                                  l->m_bIsVisible);
          } else {
...
void ODConfig::UI_Import(wxWindow* parent, bool islayer, bool isTemporary,
                         wxString dirpath, bool isdirectory) {
  int response = wxID_CANCEL;
  wxArrayString file_array;
  ...
  if (m_sImport_Type == _T("csv")) {
    m_sWildcardString =
        _T("CSV files (*.csv)|*.csv|GPX files (*.gpx)|*.gpx|All files ")
        _T("(*.*)|*.*");
  } else if (m_sImport_Type == _T("gpx")) {
    m_sWildcardString =
        _T("GPX files (*.gpx)|*.gpx|CSV files (*.csv)|*.csv|All files ")
        _T("(*.*)|*.*");
  } else if (m_sImport_Type == _T("*")) {
    m_sWildcardString =
        _T("All files (*.*)|*.*|GPX files (*.gpx)|*.gpx|CSV files ")
        _T("(*.csv)|*.csv");
  } else {
    m_sWildcardString =
        _T("GPX files (*.gpx)|*.gpx|CSV files (*.csv)|*.csv|All files ")
  ...
  wxString selectedFile;
  response =
      PlatformFileSelectorDialog(NULL, &selectedFile, _("Import GPX file"),
                                 m_sImport_Path, wxT(""), m_sWildcardString);
  if (response == wxID_OK) {
    file_array.Add(selectedFile);
  }
#else

  wxFileDialog openDialog(parent, _("Import file"), m_sImport_Path, _T(""),
                          m_sWildcardString, wxFD_OPEN | wxFD_MULTIPLE);
  openDialog.Centre();
  response = openDialog.ShowModal();
  if (response == wxID_OK) {
    openDialog.GetPaths(file_array);
    ...
    for (unsigned int i = 0; i < file_array.GetCount(); i++) {
      ...
      if (wxFileExists(path)) {
        if (islayer) {
          if (isTemporary) {
            if (i == 0) {
              ...
              if (file_array.GetCount() <= 1)
                wxFileName::SplitPath(file_array[0], NULL, NULL,
                                      &(l_pLayer->m_LayerName), NULL, NULL);
              else {
                if (dirpath.IsSameAs(_T("")))
                  wxFileName::SplitPath(m_sImport_Path, NULL, NULL,
                                        &(l_pLayer->m_LayerName), NULL, NULL);
                else
                  wxFileName::SplitPath(dirpath, NULL, NULL,
                                        &(l_pLayer->m_LayerName), NULL, NULL);
              }
              ...
              wxString laymsg;
              laymsg.Printf(_("New layer %d: %s"), l_pLayer->m_LayerID,
                            l_pLayer->m_LayerName.c_str());
              wxLogMessage(laymsg);
            ...
            if (wxCopyFile(f, destf, true)) {
              msg.Printf(
                  _T("File: %s.%s also potentially added to persistent layers"),
                  name, ext);
            ...
            ODLayerList::iterator iter;
            bool l_foundLayer = false;
            for (iter = g_pLayerList->begin(); iter != g_pLayerList->end();
                 ++iter) {
              ODLayer* current = *iter;
            ...
            wxString laymsg;
            if (l_foundLayer) {
              laymsg.Printf(wxT("Layer %d: %s is a duplicate, not added"),
                            l_pLayer->m_LayerID, l_pLayer->m_LayerName.c_str());
              wxLogMessage(laymsg);
              continue;
            } else {
              laymsg.Printf(wxT("New layer %d: %s"), l_pLayer->m_LayerID,
                            l_pLayer->m_LayerName.c_str());
              wxLogMessage(laymsg);
            }
        ...
        if (l_fn.GetExt() == _T("csv")) {
          if (l_pLayer) {
            l_pLayer->m_NoOfItems +=
                pSet->Load_CSV_File(path.fn_str(), l_pLayer->m_LayerID,
                                    l_pLayer->m_bIsVisibleOnChart);
          } else {
          ...
          pSet->load_file(path.fn_str());
          if (islayer) {
            l_pLayer->m_NoOfItems += pSet->LoadAllGPXObjectsAsLayer(
                l_pLayer->m_LayerID, l_pLayer->m_bIsVisibleOnChart);
          } else
            pSet->LoadAllGPXObjects(
                true);  // Import with full vizibility of names and objects
        }
  ...
  if (g_navobjbackups > 0) {
    ...
    if (s_diff != 0) {
      for (int i = g_navobjbackups - 1; i >= 1; i--) {
        ...
        newname =
            wxString::Format(_T("%s.%d"), m_sODNavObjSetFile.c_str(), i + 1);
