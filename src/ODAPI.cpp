...
wxArrayString ODAPI::OD_FindAllPathsGUIDS(GUIDList_t *pGL) {
  wxArrayString* l_pGUIDArray;
  l_pGUIDArray = g_pPathMan->FindAllPathsGUIDS(pGL->GUIDType, pGL->InLayer);
  wxArrayString GUID_Array(*l_pGUIDArray);
  return GUID_Array;
}

wxArrayString ODAPI::OD_FindAllPointsGUIDS(GUIDList_t *pGL) {
  wxArrayString* l_pGUIDArray;
  l_pGUIDArray = g_pODPointMan->FindAllPointsGUIDS(pGL->GUIDType, pGL->InLayer);
  wxArrayString GUID_Array(*l_pGUIDArray);
  return GUID_Array;
}

bool ODAPI::OD_FindPathByGUID(wxString sGUID, wxString* sName,
...
bool ODAPI::OD_FindPointInAnyBoundary(FindPointInAnyBoundary_t* pFPIAB) {
  ...
  if (pFPIAB->sBoundaryType == wxT("Exclusion"))
    l_BoundaryType = ID_BOUNDARY_EXCLUSION;
  else if (pFPIAB->sBoundaryType == wxT("Inclusion"))
    l_BoundaryType = ID_BOUNDARY_INCLUSION;
  else if (pFPIAB->sBoundaryType == wxT("Neither"))
    l_BoundaryType = ID_BOUNDARY_ANY;
  else
    l_BoundaryType = ID_BOUNDARY_ANY;

  if (pFPIAB->sBoundaryState == wxT("Active"))
    l_BoundaryState = ID_PATH_STATE_ACTIVE;
  else if (pFPIAB->sBoundaryState == wxT("Inactive"))
    l_BoundaryState = ID_PATH_STATE_INACTIVE;
  else if (pFPIAB->sBoundaryState == wxT("Any"))
    l_BoundaryState = ID_PATH_STATE_ANY;
  else
    l_BoundaryState = ID_BOUNDARY_ANY;

  wxString l_sGUID = g_pBoundaryMan->FindPointInBoundary(
      pFPIAB->dLat, pFPIAB->dLon, l_BoundaryType, l_BoundaryState);
  if (l_sGUID.Len() > 0) {
    ...
    l_sGUID = g_pBoundaryMan->FindPointInBoundaryPoint(
        pFPIAB->dLat, pFPIAB->dLon, l_BoundaryType);
    if (l_sGUID.Len() > 0) {
      BoundaryPoint* l_boundarypoint =
          (BoundaryPoint*)g_pODPointMan->FindODPointByGUID(l_sGUID);
...
bool ODAPI::OD_FindClosestBoundaryLineCrossing(
    FindClosestBoundaryLineCrossing_t* pFCBLC) {
  int l_BoundaryType;
  int l_BoundaryState;

  if (pFCBLC->sBoundaryType == wxT("Exclusion"))
    l_BoundaryType = ID_BOUNDARY_EXCLUSION;
  else if (pFCBLC->sBoundaryType == wxT("Inclusion"))
    l_BoundaryType = ID_BOUNDARY_INCLUSION;
  else if (pFCBLC->sBoundaryType == wxT("Neither"))
    l_BoundaryType = ID_BOUNDARY_ANY;
  else
    l_BoundaryType = ID_BOUNDARY_ANY;

  if (pFCBLC->sBoundaryState == wxT("Active"))
    l_BoundaryState = ID_PATH_STATE_ACTIVE;
  else if (pFCBLC->sBoundaryState == wxT("Inactive"))
    l_BoundaryState = ID_PATH_STATE_INACTIVE;
  else if (pFCBLC->sBoundaryState == wxT("Any"))
    l_BoundaryState = ID_PATH_STATE_ANY;
  else
    l_BoundaryState = ID_BOUNDARY_ANY;

  Boundary* l_boundary = g_pBoundaryMan->FindLineCrossingBoundaryPtr(
      pFCBLC->dStartLon, pFCBLC->dStartLat, pFCBLC->dEndLon, pFCBLC->dEndLat,
      &pFCBLC->dCrossingLon, &pFCBLC->dCrossingLat, &pFCBLC->dCrossingDistance,
      l_BoundaryType, l_BoundaryState);
  if (l_boundary != 0) {
  ...
  BoundaryPoint* l_op;
  l_op = g_pODPointMan->FindLineCrossingBoundaryPtr(
      pFCBLC->dStartLon, pFCBLC->dStartLat, pFCBLC->dEndLon, pFCBLC->dEndLat,
      l_BoundaryType, l_BoundaryState);
  if (l_op != 0) {
...
bool ODAPI::OD_FindFirstBoundaryLineCrossing(
    FindClosestBoundaryLineCrossing_t* pFCBLC) {
  int l_BoundaryType;
  int l_BoundaryState;
  ...
  Boundary* l_boundary = g_pBoundaryMan->FindLineCrossingBoundaryPtr(
      pFCBLC->dStartLon, pFCBLC->dStartLat, pFCBLC->dEndLon, pFCBLC->dEndLat,
      &pFCBLC->dCrossingLon, &pFCBLC->dCrossingLat, &pFCBLC->dCrossingDistance,
      l_BoundaryType, l_BoundaryState, true);
  if (l_boundary != 0) {
    pFCBLC->sName = l_boundary->m_PathNameString;
    pFCBLC->sDescription = l_boundary->m_PathDescription;
    pFCBLC->sGUID = l_boundary->m_GUID;
...
bool ODAPI::OD_CreateBoundary(CreateBoundary_t* pCB) {
  if (pCB->ODAPIVersionMajor > ODAPI_VERSION_MAJOR ||
      (pCB->ODAPIVersionMajor = ODAPI_VERSION_MAJOR &&
                                pCB->ODAPIVersionMinor > ODAPI_VERSION_MINOR)) {
    return false;
  }
  ...
  if (pCB->type != EXCLUSION_BOUNDARY && pCB->type != INCLUSION_BOUNDARY &&
      pCB->type != NEITHER_BOUNDARY)
    l_bValidBoundary = false;
  ...
  std::list<CreateBoundaryPoint_t*>::iterator it =
      pCB->BoundaryPointsList.begin();
  while (it != pCB->BoundaryPointsList.end()) {
    BoundaryPoint* l_pBP =
        new BoundaryPoint((*it)->lat, (*it)->lon, g_sODPointIconName,
                          (*it)->name, wxEmptyString, false);
    ...
    std::list<HyperLinkList_t*>::iterator l_it =
        (*it)->BoundaryPointHyperLinkList.begin();
...
bool ODAPI::OD_DeleteBoundary(DeleteBoundary_t* pDB) {
  if (pDB->ODAPIVersionMajor > ODAPI_VERSION_MAJOR ||
      (pDB->ODAPIVersionMajor = ODAPI_VERSION_MAJOR &&
                                pDB->ODAPIVersionMinor > ODAPI_VERSION_MINOR)) {
    return false;
  }
...
bool ODAPI::OD_CreateBoundaryPoint(CreateBoundaryPoint_t* pCBP) {
  if (pCBP->ODAPIVersionMajor > ODAPI_VERSION_MAJOR ||
      (pCBP->ODAPIVersionMajor = ODAPI_VERSION_MAJOR &&
                                pCBP->ODAPIVersionMinor > ODAPI_VERSION_MINOR)) {
    return false;
  }
  ...
  if (pCBP->iconname.IsEmpty()) {
    l_pBP = new BoundaryPoint(pCBP->lat, pCBP->lon, g_sODPointIconName,
                              pCBP->name, wxEmptyString, false);
  } else {
    l_pBP = new BoundaryPoint(pCBP->lat, pCBP->lon, pCBP->iconname, pCBP->name,
                              wxEmptyString, false);
  }
  ...
  std::list<HyperLinkList_t*>::iterator l_it =
      pCBP->BoundaryPointHyperLinkList.begin();
...
bool ODAPI::OD_DeleteBoundaryPoint(DeleteBoundaryPoint_t* pDBP) {
  if (pDBP->ODAPIVersionMajor > ODAPI_VERSION_MAJOR ||
      (pDBP->ODAPIVersionMajor = ODAPI_VERSION_MAJOR &&
                                pDBP->ODAPIVersionMinor > ODAPI_VERSION_MINOR)) {
    return false;
  }

  BoundaryPoint* plBP =
      (BoundaryPoint*)g_pODPointMan->FindODPointByGUID(pDBP->GUID);
  if (plBP) {
    if (plBP->m_bIsInBoundary) {
      ...
      if (NULL != g_pODPointMan) g_pODPointMan->RemoveODPoint(plBP);
      if (g_pPathAndPointManagerDialog &&
          g_pPathAndPointManagerDialog->IsShown())
...
bool ODAPI::OD_CreateTextPoint(CreateTextPoint_t* pCTP) {
  if (pCTP->ODAPIVersionMajor > ODAPI_VERSION_MAJOR ||
      (pCTP->ODAPIVersionMajor = ODAPI_VERSION_MAJOR &&
                                pCTP->ODAPIVersionMinor > ODAPI_VERSION_MINOR)) {
    return false;
  }
  ...
  wxString* lGUID;
  if (pCTP->GUID.length() > 0)
    lGUID = new wxString(pCTP->GUID);
  else
    lGUID = new wxString(wxEmptyString);

  if (pCTP->iconname.IsEmpty()) {
    l_pTP = new TextPoint(pCTP->lat, pCTP->lon, g_sTextPointIconName,
                          pCTP->name, *lGUID, false);
  } else {
    l_pTP = new TextPoint(pCTP->lat, pCTP->lon, pCTP->iconname, pCTP->name,
                          *lGUID, false);
  }
  ...
  if (!pCTP->defaultFont) l_pTP->m_DisplayTextFont = pCTP->TextFont;
  ...
  std::list<HyperLinkList_t*>::iterator l_it =
      pCTP->TextPointHyperLinkList.begin();
...
bool ODAPI::OD_DeleteTextPoint(DeleteTextPoint_t* pDTP) {
  if (pDTP->ODAPIVersionMajor > ODAPI_VERSION_MAJOR ||
      (pDTP->ODAPIVersionMajor = ODAPI_VERSION_MAJOR &&
                                pDTP->ODAPIVersionMinor > ODAPI_VERSION_MINOR)) {
    return false;
  }
  ...
  if (plTP) {
    ...
    if (NULL != g_pODPointMan) g_pODPointMan->RemoveODPoint(plTP);
    if (g_pPathAndPointManagerDialog && g_pPathAndPointManagerDialog->IsShown())
      g_pPathAndPointManagerDialog->UpdateODPointsListCtrl();
...
bool ODAPI::OD_AddPointIcon(AddPointIcon_t* pAPI) {
  if (pAPI->ODAPIVersionMajor > ODAPI_VERSION_MAJOR ||
      (pAPI->ODAPIVersionMajor = ODAPI_VERSION_MAJOR &&
                                pAPI->ODAPIVersionMinor > ODAPI_VERSION_MINOR)) {
    return false;
  }

  g_pODPointMan->ProcessIcon(pAPI->PointIcon, pAPI->PointIconName,
                             pAPI->PointIconDescription);
  return true;
}

bool ODAPI::OD_DeletePointIcon(DeletePointIcon_t* pDPI) {
  if (pDPI->ODAPIVersionMajor > ODAPI_VERSION_MAJOR ||
      (pDPI->ODAPIVersionMajor = ODAPI_VERSION_MAJOR &&
                                pDPI->ODAPIVersionMinor > ODAPI_VERSION_MINOR)) {
    return false;
  }
