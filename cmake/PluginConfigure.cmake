...
if(NOT SKIP_VERSION_CONFIG)
    ...
    find_file(PLUGIN_EXTRA_VERSION_VARS NAMES version.h.extra PATHS ${CMAKE_CURRENT_LIST_DIR}/in-files NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
    ...
    find_file(PLUGIN_EXTRA_FORMBUILDER_HEADERS NAMES extra_formbuilder_headers.h.in PATHS ${CMAKE_CURRENT_LIST_DIR}/in-files NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
    ...
    configure_file(${CMAKE_CURRENT_LIST_DIR}/in-files/version.h.in ${BUILD_INCLUDE_PATH}/include/version.h)
    configure_file(${CMAKE_CURRENT_LIST_DIR}/in-files/wxWTranslateCatalog.h.in ${BUILD_INCLUDE_PATH}/include/wxWTranslateCatalog.h)
...
message(STATUS "${CMLOC}PKG_NVR: ${PKG_NVR}, PKG_TARGET: ${PKG_TARGET}, ARCH: ${ARCH}, PKG_TARGET_WX_VER: ${PKG_TARGET_WX_VER}, PKG_BUILD_GTK: ${PKG_BUILD_GTK}, PKG_TARGET_VERSION: ${PKG_TARGET_VERSION}, OCPN_TARGET: $ENV{OCPN_TARGET}")
if(DEFINED ENV{OCPN_TARGET})
    ...
    if(APPLE AND CMAKE_OSX_ARCHITECTURES)
        set(PACKAGING_NAME_XML "${PKG_NVR}-${PKG_TARGET}-${COMPOUND_ARCH_DASH}-${PKG_TARGET_VERSION}${PKG_BUILD_GTK}")
    endif()
endif(DEFINED ENV{OCPN_TARGET})
if(OCPN_FLATPAK_CONFIG)
    message(STATUS "${CMLOG}Checking for flatpak extensions")
    find_file(PLUGIN_FLATPAK_ARGS NAMES flatpak_args.in PATHS ${CMAKE_CURRENT_LIST_DIR}/in-files NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
    ...
    find_file(PLUGIN_FLATPAK_OPTIONS NAMES flatpak_options.in PATHS ${CMAKE_CURRENT_LIST_DIR}/in-files NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
    ...
    find_file(PLUGIN_FLATPAK_EXTRA_MODULES NAMES flatpak_extra_modules.in PATHS ${CMAKE_CURRENT_LIST_DIR}/in-files NO_DEFAULT_PATH NO_CMAKE_FIND_ROOT_PATH)
