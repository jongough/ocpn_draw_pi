...
docker exec -ti $DOCKER_CONTAINER_ID sudo curl http://mirrordirector.raspbian.org/raspbian.public.key  | apt-key add -
docker exec -ti $DOCKER_CONTAINER_ID sudo curl http://archive.raspbian.org/raspbian.public.key  | apt-key add -
